  ...
  return (
    <div className={`quest-card quest-card-${mode}`}>
      <div className="quest-card-header">
        ...
        <span className="quest-category-tag" style={{ '--cat-color': catColor, '--cat-bg': catColor + '22' } as React.CSSProperties}>
          {catLabel}
        </span>
      </div>
    ...
    <>
      {isPendingTurnIn ? (
        <div className="quest-step-info">
          <span className="quest-step-name quest-step-name-turnin">
...
function CompletedBody({ def, completedAt, repeatCount }: QuestCardCompletedProps) {
  return (
    <>
      <div className="quest-card-desc quest-card-desc-faded">{def.description}</div>
      <div className="quest-completed-info">
        <span>{QUEST_TEXTS.completedAt(completedAt)}</span>
        {repeatCount > 1 && <span className="quest-repeat-count">{QUEST_TEXTS.repeatCount(repeatCount)}</span>}
      </div>
    </>
  );
}
